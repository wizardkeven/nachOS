...
ExceptionHandler(ExceptionType which)
{
	...
	if (which == SyscallException) {
		switch (type) {
			...
			case SC_PutChar: {
				int lecture = machine->ReadRegister (4); // on lit le registre 4 ! c'est un int
			...
			case SC_ForkExec: {
				char *recup = new char[MAX_STRING_SIZE];
				copyStringFromMachine(machine->ReadRegister(4), recup, MAX_STRING_SIZE);
				int recup2 = do_ForkExec(recup);
				if(recup2 == -1){
					printf("Erreur ForkExec\n");
					break;
				}
				
				//machine->WriteRegister(2, recup2);
				break;
			}
			default: {
				printf("Unexpected user mode exception %d %d\n", which, type);
